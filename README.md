...
| `WHISPER_HOTWORDS` | 未指定 | 認識候補を誘導する固有語（読点区切り） |
| `TRANSCRIPT_REPLACEMENTS` | `{}` | 認識後の表記を確定置換するJSON辞書 |
...
矢上祭固有の名称は、`.env`の`WHISPER_HOTWORDS`へ読点区切りで設定できます。

```env
WHISPER_HOTWORDS=矢上祭、矢上キャンパス、本部、実行委員、ステージ、警備、救護、落とし物、迷子、模擬店、トランシーバー
```

変更後はサーバーを再起動してください。固有語は強制置換ではなく認識候補へのヒントです。無関係な語を大量に追加せず、会場で頻出する語に絞ってください。

人名などの同音異字を確実に直したい場合は、ひらがな・カタカナの認識結果を`TRANSCRIPT_REPLACEMENTS`へ登録します。

```env
TRANSCRIPT_REPLACEMENTS={"なるあき":"成晃","ナルアキ":"成晃","りょうた":"遼大","リョウタ":"遼大"}
```

置換はWhisper認識後に行われ、`なるあきさん`は`成晃さん`になります。一般語の誤置換を避けるため、同じ読みの人物が複数いる場合は名字を含む表記を登録してください。
